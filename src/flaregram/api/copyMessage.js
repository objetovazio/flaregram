...
export async function f_copyMessage(body) {
  try {
    ...
    if (body.chat_id != undefined) {
      ...
      throw new Error(
        ErrorStr.undefinedParameter + colors.yellow + `chat_id` + colors.white
      );
    }
    ...
    const param_message_thread_id =
      body.message_thread_id == undefined
        ? parseInt(body.message_thread_id)
        : "";
    const param_caption = body.caption == undefined ? "" : body.caption;
    const param_parse_mode =
      body.parse_mode == undefined ? "" : body.parse_mode;
    const param_caption_entities =
      body.caption_entities == undefined ? "" : body.caption_entities;
    const param_disable_notification =
      body.disable_notification == undefined
        ? "False"
        : body.disable_notification;
    const param_protect_content =
      body.protect_content == undefined ? "False" : body.protect_content;
    const param_reply_parameters =
      body.reply_parameters == undefined ? "" : body.reply_parameters;
    const param_reply_markup =
      body.reply_markup == undefined ? "" : body.reply_markup;
        ...
        if (response.ok == false) {
          ...
          console.error(
            colors.red +
              `ERROR =>  ` +
              colors.yellow +
              `Cannot Copy Message - msg: ${payload}` +
              colors.white
          );
        }
      });
